-- =============================================
-- Entity name: StpMntAnagPianiTipo_KyDel
-- Author:		auto
-- Create date: 230126
-- AutoCreate: YES
-- Custom: NO
...
-- Description: This stored procedure is intended for deleting a specific row from  TbMntAnagPianiTipo table
-- History:
-- dav 230126 Creazione
-- =============================================
CREATE PROCEDURE [dbo].[StpMntAnagPianiTipo_KyDel]
(
...
BEGIN
	...
	*/
	...
	SET @MsgObj ='MntAnagPianiTipo'

	DECLARE @PrInfo NVARCHAR(300)
	-- SET @PrInfo = (SELECT IdPianoTipo FROM TbMntAnagPianiTipo WHERE (IdPianoTipo = @IdPianoTipo))
	-- SET @PrInfo = 'Riga ' + isnull(@PrInfo ,'--')
	-- SET @PrInfo = 'Doc ' + @IdPianoTipo
	...
	****************************************************************/
	...
	BEGIN
		...
		****************************************************************/
		/*
		IF EXISTS(SELECT IdPianoTipo FROM #TableName WHERE (IdPianoTipo = @IdPianoTipo))
		BEGIN
			...
			BEGIN TRANSACTION
				...
				BEGIN
					...
					****************************************************************/
					...
					EXECUTE StpGen_DelDoc 'TbMntAnagPianiTipo', @IdPianoTipo
