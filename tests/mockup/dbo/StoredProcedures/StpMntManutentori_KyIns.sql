...
-- ==========================================================================================
-- Entity Name:	StpMntManutentori_KyIns
-- Create date:	07/02/2018 15:39:20
-- AutoCreate:	YES
-- Custom:	NO
-- Generator:	01.00.01
-- CustomNote:	Write custom note here
-- Description:	This stored procedure is intended for inserting values to TbMntManutentori table
-- FRA 221107 - Aggiunto ISNULL per DBOH su Flag Disabilita
-- ==========================================================================================
CREATE Procedure [dbo].[StpMntManutentori_KyIns]
(
	@IdManutentore int OUTPUT,
	@IdUnita int,
	...
	BEGIN
		BEGIN TRY
			BEGIN TRANSACTION
				...
				--FRA221107 - Aggiunto ISNULL per DBOH
				IF @Disabilita IS NULL
				BEGIN
					SET @Disabilita = 0
				END

				Insert Into TbMntManutentori
					([IdUnita],[IdPianoTipo],[IdFornitore],[Manutentore],[Note],[Disabilita],[SysUserCreate],[SysDateCreate])
				Values
					(@IdUnita,@IdPianoTipo,@IdFornitore,@Manutentore,@Note,@Disabilita,@SysUserCreate,Getdate())

				Select @IdManutentore = SCOPE_IDENTITY()
