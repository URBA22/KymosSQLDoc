-- ==========================================================================================
-- Entity Name:   StpMntUnitaIntrv_KyDel
-- Author:        fab
...
-- Description:   
-- History:
-- dav 191222 Descrizione
-- sim 211018 Aggiunto controllo su eliminazione
-- ==========================================================================================
CREATE Procedure [dbo].[StpMntUnitaIntrv_KyDel](
	@IdIntrv nvarchar(20),
	...
	****************************************************************/
	IF @KYStato IS NULL OR (@KYStato = 1 and @KYRes = 1)
		BEGIN
			...
			****************************************************************/
			IF EXISTS(SELECT IdIntrvRegTmp FROM TbMntUnitaIntrvRegTmp WHERE (IdIntrv = @IdIntrv))
				BEGIN
					SET @Msg1 = 'Operazione annullata, ci sono record correlati'
					EXECUTE StpUteMsg @Msg, @Msg1, @MsgObj, @PrInfo, 'ALR', @SysUserUpdate,
					        @KYStato, @KYRes, 'Registrazioni tempi', Null, @KYMsg OUT
					SET @KYStato = -2
					RETURN
				END

			IF EXISTS(SELECT IdIntrvDist FROM TbMntUnitaIntrvDist WHERE (IdIntrv = @IdIntrv))
				BEGIN
					SET @Msg1 = 'Operazione annullata, ci sono record correlati'
					EXECUTE StpUteMsg @Msg, @Msg1, @MsgObj, @PrInfo, 'ALR', @SysUserUpdate,
					        @KYStato, @KYRes, 'Distinta', Null, @KYMsg OUT
					SET @KYStato = -2
					RETURN
				END
				...
				BEGIN TRANSACTION
					...
					ELSE
						BEGIN
							...
							EXECUTE StpGen_DelDoc 'TbMntUnitaIntrv', @IdIntrv
