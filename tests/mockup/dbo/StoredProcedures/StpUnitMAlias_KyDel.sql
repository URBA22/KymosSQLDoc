-- =============================================
-- Entity name: StpUnitMAlias_KyDel
-- Author:		auto
-- Create date: 221203
-- AutoCreate: YES
-- Custom: NO
...
-- Description: This stored procedure is intended for deleting a specific row from  TbUnitMAlias table
-- History:
-- dav 221203 Creazione
-- =============================================
CREATE PROCEDURE [dbo].[StpUnitMAlias_KyDel]
(
	@IdUnitMAlias INT,
...
BEGIN
	...
	*/
	...
	-- SET @PrInfo = (SELECT IdUnitMAlias FROM TbUnitMAlias WHERE (IdUnitMAlias = @IdUnitMAlias))
	-- SET @PrInfo = 'Riga ' + isnull(@PrInfo ,'--')
	-- SET @PrInfo = 'Doc ' + @IdUnitMAlias
	...
	****************************************************************/
	...
	BEGIN
		...
		****************************************************************/
		/*
		IF EXISTS(SELECT IdUnitMAlias FROM #TableName WHERE (IdUnitMAlias = @IdUnitMAlias))
		BEGIN
			...
			BEGIN TRANSACTION
				...
				BEGIN
					...
					****************************************************************/
					...
					EXECUTE StpGen_DelDoc 'TbUnitMAlias', @IdUnitMAlias
