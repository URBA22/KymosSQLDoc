...
-- Description: This stored procedure is intended for deleting a specific row from  TbZone table
-- History:
-- dav 221127 Creazione
-- =============================================
CREATE PROCEDURE [dbo].[StpZone_KyDel]
(
	@IdZona NVARCHAR(20),
...
BEGIN
	...
	*/
	...
	-- SET @PrInfo = (SELECT IdZona FROM TbZone WHERE (IdZona = @IdZona))
	-- SET @PrInfo = 'Riga ' + isnull(@PrInfo ,'--')
	-- SET @PrInfo = 'Doc ' + @IdZona
	...
	****************************************************************/
	...
	BEGIN
		...
		****************************************************************/
		/*
		IF EXISTS(SELECT IdZona FROM #TableName WHERE (IdZona = @IdZona))
		BEGIN
			...
			BEGIN TRANSACTION
				...
				BEGIN
					...
					****************************************************************/
					...
					EXECUTE StpGen_DelDoc 'TbZone', @IdZona
