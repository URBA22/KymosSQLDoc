-- ==========================================================================================
-- Entity Name:   StpMntUnitaIntrvRegTmp_KyDel
-- Author:        DAV
-- Create date:   18/01/2019 18:23:20
-- Custom_Dbo:	  NO
-- Standard_dbo:  YES
-- CustomNote:    
-- Description:   This stored procedure is intended for deleting a specific row from TbMntUnitaIntrvRegTmp table
-- History:
-- dav 201014 descrizione
-- ==========================================================================================
CREATE Procedure [dbo].[StpMntUnitaIntrvRegTmp_KyDel](
	@IdIntrvRegTmp int,
...
Begin
	...
	*/
	...
	SET @MsgObj = 'StpMntUnitaIntrvRegTmp_KyDel'
	...
	****************************************************************/
	IF @KYStato IS NULL OR (@KYStato = 1 and @KYRes = 1)
		BEGIN
			...
			****************************************************************/
			/*
			If Exists(SELECT IdIntrvRegTmp  FROM TbMntUnitaIntrvRegTmp WHERE (IdIntrvRegTmp = @IdIntrvRegTmp))
			BEGIN
			...
			*/
			...
			BEGIN TRY
				BEGIN TRANSACTION
					Delete TbMntUnitaIntrvRegTmp
					Where [IdIntrvRegTmp] = @IdIntrvRegTmp
					...
					ELSE
						BEGIN
							...
							EXECUTE StpGen_DelDoc 'TbMntUnitaIntrvRegTmp', @IdIntrvRegTmp
