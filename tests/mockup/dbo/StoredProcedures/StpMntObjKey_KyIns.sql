...
-- ==========================================================================================
-- Entity Name:	StpMntObjKey_KyIns
-- Create date:	17/07/2019 11:38:12
-- AutoCreate:	YES
-- Custom:	NO
-- Generator:	01.00.01
-- CustomNote:	Write custom note here
-- Description:	This stored procedure is intended for inserting values to TbMntObjKey table
-- History:
-- fab 23.07.19	Aggiunto Value e UPDATE TbSettingKey
-- ==========================================================================================
CREATE Procedure [dbo].[StpMntObjKey_KyIns]
(
	@IdObjKey int OUTPUT,
	@IdKey nvarchar(200),
		...
		BEGIN TRY
			BEGIN TRANSACTION
				...
				Insert Into TbMntObjKey
					([IdKey],[IdObject],[SysUserCreate],[SysDateCreate])
				Values
					(@IdKey,@IdObject,@SysUserCreate,Getdate())

				Select @IdObjKey = SCOPE_IDENTITY() 
				
				UPDATE TbSettingKey
				SET Value = @Value
				WHERE IdKey = @IdKey AND ISNULL(@Value,'') <> ''
